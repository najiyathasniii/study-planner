...
async function addNewTask() {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/api/tasks`, {
            ...
            body: JSON.stringify({ title, completed: false, date: fullDateTimeStr })
        });
...
function updateDashboard() {
    ...
    updateText('sideTotal', total);
    updateText('sideCompleted', completed);
    updateText('sidePending', pending);
    updateText('analyticsRate', `${percentage}%`);
    updateText('analyticsCompleted', completed);
    updateText('analyticsPending', pending);
...
async function fetchExamsFromBackend() {
    ...
    if (!token) return;
    try {
        const response = await fetch(`${API_BASE_URL}/api/exams`, { headers: { 'Authorization': `Bearer ${token}` } });
        if (response.ok) {
...
async function addNewExam() {
    ...
    if (!title || !rawDate) { alert('Please fill in both the Title and the Due Date!'); return; }

    const parsedDate = new Date(rawDate);
    if (isNaN(parsedDate.getTime())) { alert('Please enter a valid date!'); return; }
    ...
    try {
        ...
        if (response.ok) {
            ...
            alert(errData.error || errData.message || 'Failed to add exam.');
        }
...
function renderExams() {
    ...
    examItems.sort((a, b) => new Date(a.dueDate) - new Date(b.dueDate)).forEach(item => {
        const badge = getDaysRemainingText(item.dueDate);
        const card = document.createElement('div');
        card.style.cssText = `display: flex; justify-content: space-between; align-items: center; padding: 12px 16px; background: #f9fafb; border-radius: 8px; border-left: 4px solid ${badge.color}; margin-bottom: 8px;`;
        card.innerHTML = `
            <div>
...
async function fetchNotesFromBackend() {
    ...
    if (!token) return;
    try {
        const response = await fetch(`${API_BASE_URL}/api/notes`, { headers: { 'Authorization': `Bearer ${token}` } });
        if (response.ok) {
...
async function addPdfNote() {
    ...
    if (!title || !currentSelectedFile) { alert('Please enter a title AND select a PDF file to upload!'); return; }

    const reader = new FileReader();
    reader.onload = async function (e) {
        const token = getAuthToken();
        const payload = {
            title: title, subject: subject, fileName: currentSelectedFile.name,
            fileType: currentSelectedFile.type, fileData: e.target.result,
        ...
        try {
            ...
            } else { alert('Failed to upload PDF. File size may be too large.'); }
        } catch (error) { alert('Server error occurred during PDF upload.'); }
    };
...
function renderNotes() {
    ...
    studyNotes.forEach(note => {
        const card = document.createElement('div');
        card.style.cssText = `background: #ffffff; border-radius: 10px; padding: 16px; box-shadow: 0 2px 8px rgba(0,0,0,0.06); border-left: 5px solid #2563eb; display: flex; flex-direction: column; justify-content: space-between;`;
        card.innerHTML = `
            <div>
                ...
                <p style="font-size: 12px; color: #6b7280; display: flex; align-items: center; gap: 5px;"><i class="fa-solid fa-file-pdf" style="color: #ef4444; font-size: 16px;"></i> ${note.fileName}</p>
            </div>
            <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 15px;">
                <a href="${note.fileData}" download="${note.fileName}" style="background: #2563eb; color: white; padding: 6px 12px; border-radius: 6px; text-decoration: none; font-size: 12px; font-weight: 600;">Download</a>
                <button onclick="deleteNote('${note._id}')" style="background: none; border: none; color: #ef4444; cursor: pointer; font-size: 13px;"><i class="fa-solid fa-trash"></i></button>
            </div>
        `;
...
function initCalendar() {
    ...
    calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        editable: true, droppable: true, selectable: true, selectLongPressDelay: 200, selectMirror: true,
        allDayMaintainDuration: false, defaultTimedEventDuration: '01:00:00',
        select: async function(info) {
            ...
            try {
                const response = await fetch(`${API_BASE_URL}/api/tasks`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
                    body: JSON.stringify({ title: title.trim(), completed: false, date: info.startStr })
                });
                if (response.ok) fetchTasksFromBackend();
            } catch (error) { console.error('Error adding task from calendar:', error); }
        },
        events: []
    });
    calendar.render();
...
function updateCalendarEvents() {
    ...
    tasks.forEach(task => {
        ...
        calendar.addEvent({
            id: task._id, title: task.title, start: task.date || new Date().toISOString(),
            allDay: !hasTime, color: task.completed ? '#10b981' : '#2563eb'
        });
    });
}
...
function initCharts() {
    ...
    if (weeklyCtx) {
        weeklyChart = new Chart(weeklyCtx, {
            type: 'bar',
            data: { labels: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'], datasets: [{ label: 'Tasks Completed', data: getWeeklyTaskCounts(), backgroundColor: '#2563eb', borderRadius: 4 }] },
    ...
    if (statusCtx) {
        ...
        statusChart = new Chart(statusCtx, {
            type: 'doughnut',
            data: { labels: ['Completed', 'Pending'], datasets: [{ data: [completed, tasks.length - completed], backgroundColor: ['#10b981', '#f59e0b'] }] },
...
function updateCharts() {
    if (weeklyChart) { weeklyChart.data.datasets[0].data = getWeeklyTaskCounts(); weeklyChart.update(); }
    if (statusChart) {
        const completed = tasks.filter(t => t.completed).length;
        statusChart.data.datasets[0].data = [completed, tasks.length - completed];
        statusChart.update();
    }
}
    ...
    if (token) {
        try {
            ...
            if (response.ok) {
                ...
                if (data.name) { userName = data.name; localStorage.setItem('userName', data.name); }
                if (data.email) { userEmail = data.email; localStorage.setItem('userEmail', data.email); }
                if (data.avatar) { savedAvatar = data.avatar; localStorage.setItem('userAvatar', data.avatar); }
            }
        } catch (e) { console.warn('Could not fetch remote profile:', e); }
    }

    const headerTitle = document.querySelector('header h1');
    if (headerTitle && headerTitle.textContent.includes('Hello')) headerTitle.innerHTML = `Hello, ${userName} 👋`;
...
// Global functions for Login / Register (Allows HTML inline onclick to work)
async function submitLogin(e) {
    ...
    try {
        ...
        } else { alert(data.error || data.message || 'Invalid email or password'); }
    } catch (error) { alert('Unable to connect to server. Please try again.'); }
}
...
// ==========================================
// BOOT-UP SEQUENCE (RUNS AUTOMATICALLY ON PAGE LOAD)
// ==========================================
document.addEventListener('DOMContentLoaded', () => {
    
    // 1. Attach listeners to Auth forms if they exist on the page
    ...
    const isDashboard = document.getElementById('taskList') || document.getElementById('calendar');
    ...
    if (isDashboard) {
        ...
        if (firstTab) showSection('dashboard-sec', firstTab);
    }
});
